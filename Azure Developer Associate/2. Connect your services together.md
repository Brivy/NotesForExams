...
## Enable reliable messaging for Big Data applications using Azure Event

Big data apps must be able to process increased throughput by scaling out to meet increased transaction volumes.

### Azure Event Hub explained

**Azure Event Hubs** is a cloud-based, event-processing service that can receive and process millions of events per second. Event Hubs acts as a front door for an event pipeline, to receive incoming data and stores this data until processing resources are available. The following components are important:

- The *publisher*. This is the entity that sends the data to Event Hubs.
- The *consumer/subscriber*. This is the entity that reads the data from Event Hubs.
- The *event*. A small packet of information (a *datagram*) that contains a notification. Can't exceed 1 MB in size.

![Cool picture](Pictures/2-event-hub-overview.png)

An event can be published by:

- *Advanced Message Queuing Protocol (AMQP) 1.0*: If data is send frequently, this has better performance. This is because of a persistent bidirectional socket and TLS or SSL/TLS.
- *HTTPS*: If data is send intermittent, this is the better option. It has some more overhead for each request than AMQP.
...
For creating a new Azure Event Hubs, we first need to define a Event Hubs namespace. All settings here will apply to all the Event Hubs within that namespace. The default value is 1 for capacity and Standard for pricing tier.

First, define a namespace like this: *namespace.servicebus.windows.net*.
Secondly, define the following optional properties:

- Enabling Kafka. This option enables Kafka apps to publish events to the Event Hub.
- Make the namespace zone redundant. Replicates data across separate data centers.
- Enable Auto-Inflate and Auto-Inflate Maximum Throughput Units.Auto-Inflate provides an automatic scale-up option by increasing the number of throughput units up to a maximum value. Useful to avoid throttling.

Creating an Event Hubs namespace can be done with the `az eventhubs namespace` commands. Two important ones are:

- `create`: Creates the Event Hubs namespace.
- `authorization-rule`: Returns the connection string for your Event Hubs namespace.

For creating an Event Hub, you'll need to fill in some parameters:

- **Event Hub name**: Unique within your subscription and:
  - Is between 1 and 50 characters long.
  - Contains only letters, numbers, periods, hyphens, and underscores.
  - Starts and ends with a letter or number.
- **Partition Count (default: 4)**: The partition count should be directly related to the expected number of concurrent consumers and can't be changed after the hub has been created. The partition separates the message stream so that consumer or receiver apps only need to read a specific subset of the data stream.
- **Message Retention (default: 7)**: The number of days (between 1 and 7) that messages will remain available if the data stream needs to be replayed for any reason.

Creating an Event Hubs can be done with the `az eventhubs eventhub` commands. Two important ones are:

- `create`: Creates the Event Hub in a specified namespace.
- `authorization-rule`: Displays the details of your Event Hub.

### Send and receive through Event Hub

You need to provide the following information if you want to **send** messages, so that the application can create connection credentials:
...
You need to provide the following information if you want to **receive** messages, so that the application can create connection credentials:
...
You can monitor whether all sender messages are successfully transmitted and received by using the metrics of your namespace in the Azure portal. Other useful metrics are:
...
### Responding to Azure events by using Event Grid
...
![Cool picture](Pictures/2-eventgrid-pub-sub.svg)
