...
10. Creating a Dockerfile that will build and publish the container image to an Azure Container Registry (ACR).
...
11. Encrypt a VHD and create a virtual machine when deploying on Azure.
...
12. Create a ARM template from a resource group that you will use as your deployment source by PowerShell.
...
13. Uploading an on-premise Windows VM to the cloud and encrypt it.
...
15. Configure and access a Linux VM from your local machine.
...
16. Publish the most recent Windows Core image into a ACR that is available in the Microsoft Container Registry.
...
17. Create a ARM template based on the current state of resource group myRG and its resources.
...
18. Creating a generalized vm image.
...
19. Creating a flexible ARM template with the same configuration but a different name.
...
20. Enable the generation of (queryable) logs by an Linux Azure App Service.
